...
export const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Background Decorative Ambient Radial Gradients (Lopes Red Accent) */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#F10F4D]/15 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-rose-950/30 rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-md w-full bg-slate-900/95 border border-slate-800/90 rounded-3xl p-8 sm:p-10 shadow-2xl relative z-10 backdrop-blur-xl">
        
        {/* Brand Header */}
        <div className="text-center mb-8 flex flex-col items-center">
          <LopesLogo size="lg" variant="white" showBadge badgeText="CAPTAÇÃO" className="mb-2.5" />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs font-medium text-white placeholder-slate-600 focus:outline-none focus:border-[#F10F4D] focus:ring-1 focus:ring-[#F10F4D] transition"
                required
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-3 py-3.5 bg-[#F10F4D] hover:bg-rose-600 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-rose-950/50 transition transform active:scale-95 cursor-pointer disabled:opacity-50"
          >
        ...
        {/* Recovery Dialog */}
        {showForgotMsg && (
          <div className="mt-5 p-4 bg-slate-950 border border-slate-800 rounded-2xl text-xs text-slate-300 space-y-2">
        ...
        <div className="mt-8 pt-5 border-t border-slate-800/80 text-center text-[11px] text-slate-500">
          ...
          <p className="text-[10px] text-slate-600 mt-0.5">Todos os direitos reservados</p>
        </div>

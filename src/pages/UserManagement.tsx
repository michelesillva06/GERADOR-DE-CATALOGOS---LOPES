...
import { Users, UserPlus, Edit3, ShieldAlert, KeyRound, Trash2, ExternalLink, Search, CheckCircle, Ban, X, Upload, User as UserIcon, Camera } from 'lucide-react';
...
export const UserManagement: React.FC<UserManagementProps> = ({
  ...
  const filteredUsers = users.filter(u => {
    // Non-Master Admin users (e.g. Larissa/Gestora) cannot view or alter Master Admin
    if (!isMasterUser && (u.role === 'MASTER_ADMIN' || u.username === 'admin')) {
      return false;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Controle permissões, crie novas contas e gerencie páginas públicas dos captadores
          </p>
        </div>
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl border border-slate-200 relative">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
              <h2 className="text-xl font-extrabold text-slate-900">
                {editingUser ? 'Editar Usuário Captador' : 'Cadastrar Novo Usuário'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-700 uppercase mb-1">
                  URL Pública Personalizada (/catalogo/suaurl)
                </label>
                <input
                  type="text"
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end space-x-3">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-6 py-2 bg-[#F10F4D] hover:bg-rose-600 text-white rounded-xl text-xs font-bold shadow"
                >
